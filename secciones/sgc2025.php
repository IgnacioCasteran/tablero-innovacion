...
/* ---------- Config ---------- */
$BASE_DIR = realpath(__DIR__ . '/../uploads/sgc2025');
if ($BASE_DIR === false) {
    mkdir(__DIR__ . '/../uploads/sgc2025', 0777, true);
    $BASE_DIR = realpath(__DIR__ . '/../uploads/sgc2025');
}
...
function clean_name_with_dot($s)
{
    $s = str_replace('+', ' ', $s);
    $s = trim($s);
    $s = preg_replace('/[^\pL\pN _\.\-,]/u', '', $s); // permite letras (con acentos), números, espacio, _ . - ,
    return $s !== '' ? $s : 'nombre';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $qs = ['path' => $subpath];
    if ($q !== '') $qs['q'] = $q;
    header("Location: sgc2025.php?" . http_build_query($qs));
    exit();
}
...
<body>
    <div class="container wrap">
        ...
        <h2 class="text-center mb-3 title">SGC 2025</h2>
            ...
            <div class="row g-3 align-items-stretch">
                <div class="col-12 col-md-6">
                    <form method="post" class="d-flex gap-2">
                        ...
                        <input type="text" class="form-control" name="folder_name" placeholder="Nombre de carpeta… (p.ej. Auditorías 2025 / Indicadores / Actas)" required>
                ...
                <div class="col-12">
                    <form method="get" class="d-flex search" onsubmit="/* permite también filtrar servidor */">
                        ...
                        <?php if ($q !== ''): ?>
                            <a class="btn btn-outline-dark ms-2" href="sgc2025.php?<?php echo http_build_query(['path' => $subpath]); ?>">Limpiar</a>
                        <?php endif; ?>
                    </form>
        ...
        <div class="alert alert-light border d-flex flex-wrap align-items-center gap-2 mt-0">
            ...
            <span class="badge rounded-pill badge-path"><a class="text-decoration-none" href="sgc2025.php">/ SGC 2025</a></span>
            <?php foreach ($crumbs as $c): ?>
                <span class="badge rounded-pill badge-path">
                    <a class="text-decoration-none" href="sgc2025.php?<?php $qs = ['path' => $c['path']];
                                                                        if ($q !== '') $qs['q'] = $q;
            ...
            <?php if ($subpath): ?>
                <a class="ms-auto btn btn-sm btn-outline-secondary"
                    href="sgc2025.php?<?php $qs = ['path' => dirname($subpath) === '.' ? '' : dirname($subpath)];
                                        if ($q !== '') $qs['q'] = $q;
                        ...
                        <div class="grid py-2 border-bottom js-row">
                            <div class="d-flex align-items-center gap-2">
                                <?php if ($it['isDir']): ?>
                                    ...
                                    <a class="text-decoration-none" href="sgc2025.php?<?php
                            ...
                            <div class="actions">
                                <?php if ($it['isDir']): ?>
                                    <a class="btn btn-sm btn-outline-primary"
                                        href="sgc2025.php?<?php $qs = ['path' => ($subpath ? $subpath . '/' : '') . $it['name']];
                                                            if ($q !== '') $qs['q'] = $q;
                                                            echo http_build_query($qs); ?>">
                                        Abrir
                                    </a>
                                ...
                                <?php else: ?>
                                    ...
                                    $href = "../uploads/sgc2025/" . $encodedPath;
                                    ?>
    ...
    <script>
        ...
        (function() {
            ...
            // Precalcular nombre normalizado por fila
            rows.forEach(r => {
