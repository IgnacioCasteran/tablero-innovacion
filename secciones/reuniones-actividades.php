...
<head>
    ...
    <style>
        ...
        thead tr.filters .form-select { font-size:.85rem; padding:.15rem .4rem; }
        ...
        @media (min-width:768px){
            .section-title{margin:0 0 20px 0;border-left:5px solid #7c1c2c;border-radius:4px}
        }
    </style>
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function fillSelect(select, values, current, addOtro = false) {
            ...
            if (addOtro) {
                const opt = document.createElement('option');
                opt.value = '__OTRO__'; opt.textContent = 'Otro (especificar)';
                select.appendChild(opt);
            }
            if (current && !values.includes(current)) {
                const opt = document.createElement('option');
                opt.value = current; opt.textContent = `${current} (existente)`;
                select.appendChild(opt);
            }
        ...
        function normalizarEstado(s) {
            s = norm(s);
            const map = {'completado':'Completado','en curso':'En curso','no iniciada':'No iniciada','bloqueada':'Bloqueada'};
            return map[s] || null;
        }
                ...
                if (data.tipo !== 'reunion') {
                    ...
                    if (sel) {
                        if (![...sel.options].some(o => o.value === canon)) {
                            const opt = document.createElement('option');
                            opt.value = canon; opt.textContent = canon; sel.appendChild(opt);
                        }
                        sel.value = canon;
                    }
                } else {
                    ...
                    if (sel && actual && ![...sel.options].some(o => o.value === actual)) {
                        const opt = document.createElement('option');
                        opt.value = actual; opt.textContent = `${actual} (existente)`; sel.appendChild(opt);
                        sel.value = actual;
                    }
                }
        ...
        document.getElementById("formEditar")?.addEventListener("submit", function(e) {
            ...
            fetch('../api/api-reuniones.php', { method:'POST', body:fd })
                .then(res => res.json())
        ...
        document.querySelectorAll(".btn-eliminar").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                Swal.fire({
                    ...
                    if (r.isConfirmed) {
                        fetch('../api/api-reuniones.php', { method:'DELETE', body:new URLSearchParams({id}) })
                          .then(res=>res.json())
        ...
        document.querySelectorAll('.btn-pin').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    location.reload();
                } catch(e){ Swal.fire('Error', e.message || 'No se pudo actualizar la prioridad','error'); }
            });
        });
        ...
        function attachFiltersActividades() {
            ...
            optVacio.value = '__vacio__'; optVacio.textContent = '— / Vacío';
            ...
            function apply() {
                ...
                table.querySelectorAll('tbody tr').forEach(tr => {
                    ...
                    if (o) {
                        if (o === '__vacio__') { if (cOrg) ok = false; }
