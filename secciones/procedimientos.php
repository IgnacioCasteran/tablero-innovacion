...
if ($BASE_DIR === false) {
  mkdir(__DIR__ . '/../uploads/procedimientos', 0777, true);
  $BASE_DIR = realpath(__DIR__ . '/../uploads/procedimientos');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $qs = ['path' => $subpath];
  if ($q !== '') $qs['q'] = $q;
  header("Location: procedimientos.php?" . http_build_query($qs));
  exit();
}
...
<head>
  <meta charset="UTF-8">
  <title>Procedimientos · SGC</title>
...
<body>
  <div class="container wrap">
    ...
    <h2 class="text-center mb-3 title">Procedimientos</h2>
      ...
      <div class="row g-3 align-items-stretch">
        <div class="col-12 col-md-6">
          <form method="post" class="d-flex gap-2">
            ...
            <input type="text" class="form-control" name="folder_name" placeholder="Nombre de carpeta… (p.ej. Vigentes / Obsoletos / P-01)" required>
        ...
        <div class="col-12">
          <form method="get" class="d-flex search">
            ...
            <?php if ($q !== ''): ?>
              <a class="btn btn-outline-dark ms-2" href="procedimientos.php?<?php echo http_build_query(['path' => $subpath]); ?>">Limpiar</a>
            <?php endif; ?>
          </form>
        </div>
      </div>
    ...
    <div class="alert alert-light border d-flex flex-wrap align-items-center gap-2 mt-0">
      ...
      <span class="badge rounded-pill badge-path"><a class="text-decoration-none" href="procedimientos.php">/ Procedimientos</a></span>
      <?php foreach ($crumbs as $c): ?>
        <span class="badge rounded-pill badge-path">
          <a class="text-decoration-none" href="procedimientos.php?<?php $qs = ['path' => $c['path']];
                                                                    if ($q !== '') $qs['q'] = $q;
      ...
      <?php if ($subpath): ?>
        <a class="ms-auto btn btn-sm btn-outline-secondary"
          href="procedimientos.php?<?php $qs = ['path' => dirname($subpath) === '.' ? '' : dirname($subpath)];
                                    if ($q !== '') $qs['q'] = $q;
            ...
            <div class="grid py-2 border-bottom js-row">
              <div class="d-flex align-items-center gap-2">
                <?php if ($it['isDir']): ?>
                  ...
                  <a class="text-decoration-none" href="procedimientos.php?<?php
              ...
              <div class="actions">
                <?php if ($it['isDir']): ?>
                  <a class="btn btn-sm btn-outline-primary"
                    href="procedimientos.php?<?php $qs = ['path' => ($subpath ? $subpath . '/' : '') . $it['name']];
                                              if ($q !== '') $qs['q'] = $q;
                                              echo http_build_query($qs); ?>">
                    Abrir
                  </a>
                ...
                <?php else: ?>
                  ...
                  $href = "../uploads/procedimientos/" . $encodedPath;
                  ?>
